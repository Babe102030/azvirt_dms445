...
/**
 * Export quality tests to Excel with customizable columns
 */
export async function exportQualityTestsToExcel(
  options: ExportOptions = {},
): Promise<Buffer> {
  const workbook = new ExcelJS.Workbook();
  ...
  tests.forEach((test: any) => {
    const row: any = {};
    selectedColumns.forEach((col) => {
      row[col.key] = (test as any)[col.key];
    });
    worksheet.addRow(row);
  });
...
/**
 * Export purchase orders to Excel with customizable columns
 */
export async function exportPurchaseOrdersToExcel(
  ...
  orders.forEach((order: any) => {
    const row: any = {};
    selectedColumns.forEach((col) => {
      row[col.key] = (order as any)[col.key];
    });
    worksheet.addRow(row);
  });
...
export async function exportAllDataToExcel(
  options: Record<string, ExportOptions> = {},
): Promise<Buffer> {
  const workbook = new ExcelJS.Workbook();
  ...
  // Add Quality Tests sheet
  const qualitySheet = workbook.addWorksheet("Quality Tests");
  const qualityColumns: ExportColumn[] = [
    { key: "id", header: "ID", width: 10 },
    { key: "testName", header: "Test Name", width: 30 },
    { key: "testType", header: "Test Type", width: 15 },
    { key: "result", header: "Result", width: 15 },
  ...
  const tests = await db.getQualityTests();
  tests.forEach((t: any) => qualitySheet.addRow(t));
  ...
  const orders = await db.getPurchaseOrders();
  orders.forEach((o: any) => poSheet.addRow(o));
