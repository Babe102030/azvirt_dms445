...
describe('Trigger Evaluation Engine', () => {
  describe('evaluateCondition', () => {
    ...
    it('should evaluate not_contains operator correctly', () => {
      ...
      expect(evaluateCondition(condition, { tags: 'active, pending' })).toBe(true);
      expect(evaluateCondition(condition, { tags: 'archived, old' })).toBe(false);
    });
    ...
    it('should evaluate ends_with operator correctly', () => {
      ...
      expect(evaluateCondition(condition, { filename: 'document.pdf' })).toBe(true);
      expect(evaluateCondition(condition, { filename: 'image.jpg' })).toBe(false);
    });
  ...
  describe('evaluateConditions', () => {
    ...
    it('should evaluate complex conditions with multiple groups (AND)', () => {
        ...
        groups: [
          {
            ...
            conditions: [
              ...
              { field: 'minStock', operator: 'greater_than' as const, value: 20 },
            ],
          },
          {
            operator: 'OR' as const,
            conditions: [
              { field: 'category', operator: 'equals' as const, value: 'critical' },
              { field: 'daysSinceOrder', operator: 'greater_than' as const, value: 7 },
            ],
          },
        ],
      };
      ...
      // Both groups pass
      const data1 = { stock: 30, minStock: 25, category: 'critical', daysSinceOrder: 3 };
      expect(evaluateConditions(conditions, data1)).toBe(true);
      
      // First group passes, second group fails
      const data2 = { stock: 30, minStock: 25, category: 'normal', daysSinceOrder: 3 };
      expect(evaluateConditions(conditions, data2)).toBe(false);
    });
        ...
        groups: [
          {
            operator: 'AND' as const,
            conditions: [
              { field: 'priority', operator: 'equals' as const, value: 'urgent' },
          ...
          },
          {
            operator: 'AND' as const,
            conditions: [
              { field: 'daysOverdue', operator: 'greater_than' as const, value: 14 },
            ],
          },
        ],
      };
  ...
  describe('substituteVariables', () => {
    ...
    it('should convert non-string values to strings', () => {
      const template = 'Quantity: {{quantity}}, Price: {{price}}, Active: {{active}}';
    ...
    it('should handle complex nested paths', () => {
      ...
      const data = { project: { details: { name: 'Alpha' }, status: 'active' } };
      const result = substituteVariables(template, data);
  ...
  describe('Real-world scenarios', () => {
    it('should evaluate low stock alert condition', () => {
        ...
        groups: [
          {
            operator: 'AND' as const,
            conditions: [
              { field: 'currentStock', operator: 'less_than' as const, value: 50 },
              { field: 'minStock', operator: 'greater_than' as const, value: 0 },
            ],
          },
        ],
      };
      ...
      const template = 'Low stock alert: {{materialName}} is below minimum. Current: {{currentStock}}{{unit}}, Minimum: {{minStock}}{{unit}}';
      const message = substituteVariables(template, materialData);
      expect(message).toBe('Low stock alert: Cement is below minimum. Current: 30kg, Minimum: 50kg');
    });
    ...
    it('should evaluate overdue task condition', () => {
        ...
        groups: [
          {
            ...
            conditions: [
              { field: 'priority', operator: 'equals' as const, value: 'urgent' },
              { field: 'daysOverdue', operator: 'greater_than' as const, value: 0 },
            ],
          },
          {
            operator: 'AND' as const,
            conditions: [
              { field: 'daysOverdue', operator: 'greater_than' as const, value: 7 },
            ],
          },
        ],
      };
      ...
      const template = 'Task "{{taskName}}" is {{daysOverdue}} days overdue. Assigned to: {{assignedTo}}';
      const message1 = substituteVariables(template, taskData1);
      expect(message1).toBe('Task "Fix critical bug" is 1 days overdue. Assigned to: John');
    });
    ...
    it('should evaluate delivery delay condition', () => {
        ...
        groups: [
          {
            operator: 'AND' as const,
            conditions: [
              { field: 'status', operator: 'equals' as const, value: 'delayed' },
              { field: 'delayHours', operator: 'greater_than' as const, value: 2 },
            ],
          },
        ],
      };
      ...
      const template = 'Delivery #{{deliveryId}} to {{projectName}} is delayed by {{delayHours}} hours. Driver: {{driverName}}';
      const message = substituteVariables(template, deliveryData);
      expect(message).toBe('Delivery #123 to Building Alpha is delayed by 4 hours. Driver: Mike');
    });
    ...
    it('should evaluate quality test failure condition', () => {
        ...
        groups: [
          {
            ...
            conditions: [
              ...
              { field: 'testType', operator: 'equals' as const, value: 'strength' },
            ],
          },
        ],
      };
      ...
      const template = 'Quality Test #{{testId}} FAILED: {{testType}} test for {{projectName}}. Tested by: {{testedBy}}';
      const message = substituteVariables(template, testData);
      expect(message).toBe('Quality Test #456 FAILED: strength test for Tower Construction. Tested by: QC Team');
    });
  });
});
