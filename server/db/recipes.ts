...
import { getDb } from "../db"; // Deprecated, will remove after ensuring no partial deps
// Temporary types
...
export async function getRecipeIngredients(recipeId: number) {
  ...
  try {
    ...
    return result.records.map(r => {
      ...
      const rel = r.get('rel');
      return {
        id: rel.properties.id, // Relationship property? Or synthesized?
        recipeId,
        materialId: material.id,
        materialName: material.name, // Enriching return
