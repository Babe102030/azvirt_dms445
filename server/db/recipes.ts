...
export async function getAllRecipes() {
  ...
  try {
    return await db
      .select()
      .from(concreteRecipes)
      .orderBy(concreteRecipes.name);
  } catch (error) {
...
export async function getRecipeById(id: number) {
  ...
  try {
    const recipes = await db
      .select()
      .from(concreteRecipes)
      .where(eq(concreteRecipes.id, id));
 ...
 */
export async function addRecipeIngredient(ingredient: any) {
  const db = await getDb();
  if (!db) return false;
 ...
 */
export async function calculateRecipeQuantities(
  recipeId: number,
  targetVolume: number,
) {
  ...
  const yieldVolume = Number(recipe.yieldVolume) || 1000;
  const multiplier = targetVolume / yieldVolume;
  ...
  return {
    ...
    ingredients: ingredients.map((ingredient) => ({
      ...ingredient,
      calculatedQuantity: Math.ceil(Number(ingredient.quantity) * multiplier), // Round up to avoid shortages
    })),
  };
}
