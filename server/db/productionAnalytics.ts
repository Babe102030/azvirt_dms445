...
export async function getDailyProductionVolume(days: number = 30) {
  ...
  try {
    ...
    const volumeByDate: Record<
      string,
      { date: string; volume: number; count: number }
    > = {};

    for (const batch of batches) {
      if (!batch.createdAt) continue;
      ...
      volumeByDate[dateStr].volume += Number(batch.volume) || 0;
      volumeByDate[dateStr].count += 1;
    }

    return Object.values(volumeByDate).sort((a, b) =>
...
export async function getMaterialConsumptionTrends(days: number = 30) {
  ...
  try {
    ...
    const consumptionByMaterial: Record<
      number,
      { materialId: number; totalQuantity: number; unit: string; name: string }
    > = {};
    ...
    for (const batch of completedBatches) {
      ...
      for (const ingredient of ingredients) {
        if (ingredient.materialId) {
          ...
          consumptionByMaterial[ingredient.materialId].totalQuantity +=
            Number(ingredient.plannedQuantity) || 0;
        }
      }
    }
...
export async function getProductionEfficiencyMetrics(days: number = 30) {
  ...
  try {
    ...
    const successRate =
      totalBatches > 0 ? (completedBatches.length / totalBatches) * 100 : 0;
    const totalVolume = completedBatches.reduce(
      (sum: number, b: any) => sum + (Number(b.volume) || 0),
      0,
    );
    const avgBatchVolume =
      completedBatches.length > 0 ? totalVolume / completedBatches.length : 0;
    ...
    const avgBatchTime =
      batchesWithTime > 0 ? totalBatchTime / batchesWithTime : 0;
...
export async function getProductionByRecipe(days: number = 30) {
  ...
  try {
    ...
    // Group by recipe
    const volumeByRecipe: Record<
      number,
      { recipeId: number; recipeName: string; volume: number; count: number }
    > = {};
    ...
    for (const batch of batches) {
      if (batch.recipeId) {
        ...
        volumeByRecipe[batch.recipeId].volume += Number(batch.volume) || 0;
        volumeByRecipe[batch.recipeId].count += 1;
      }
    }

    return Object.values(volumeByRecipe).sort(
...
export async function getHourlyProductionRate(days: number = 7) {
  ...
  try {
    ...
    for (const batch of completedBatches) {
      if (!batch.createdAt) continue;
      const hour =
        batch.createdAt.toISOString().split("T")[0] +
        " " +
        String(batch.createdAt.getHours()).padStart(2, "0") +
        ":00";
    ...
    return Object.values(rateByHour).sort((a, b) =>
