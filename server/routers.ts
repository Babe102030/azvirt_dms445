...
export const appRouter = router({
  ...
  materials: router({
    ...
    sendLowStockAlert: protectedProcedure.mutation(async () => {
      ...
      const materialsList = lowStockMaterials
        .map(
          (m) =>
            `- ${m.name}: ${m.quantity} ${m.unit} (minimum: ${m.minStock} ${m.unit})`,
        )
        .join("\n");
    ...
    sendCriticalStockSMS: protectedProcedure.mutation(async () => {
      ...
      const materialsList = criticalMaterials
        .map(
          (m: any) =>
            `${m.name}: ${m.quantity}/${m.criticalThreshold} ${m.unit}`,
        )
        .join(", ");
  ...
  deliveries: router({
    ...
    uploadDeliveryPhoto: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (delivery) {
          const existingPhotos = delivery.deliveryPhotos
     ...
     */
    updateStatusWithGPS: protectedProcedure
      .input(
        z.object({
          deliveryId: z.number(),
          status: z.enum([
          ...
          gpsLocation: z.string().optional(), // "lat,lng" format
          driverNotes: z.string().optional(),
        }),
      )
      .mutation(async ({ input, ctx }) => {
        try {
          const result = await db.updateDeliveryStatusWithGPS(
            input.deliveryId,
            input.status,
            input.gpsLocation,
            input.driverNotes,
    ...
    getHistory: protectedProcedure
      ...
      .query(async ({ input }) => {
        return await db.getDeliveryHistory(input.deliveryId);
      }),
    ...
    calculateETA: protectedProcedure
      .input(
        z.object({
          deliveryId: z.number(),
          currentGPS: z.string().optional(), // "lat,lng" format
        }),
      )
      .mutation(async ({ input }) => {
        const eta = await db.calculateDeliveryETA(
          input.deliveryId,
          input.currentGPS,
        );
  ...
  dashboard: router({
    stats: protectedProcedure.query(async () => {
      const [allProjects, allDocuments, allMaterials, allDeliveries, allTests] =
        await Promise.all([
          db.getProjects(),
          db.getDocuments(),
          db.getMaterials(),
    ...
    deliveryTrends: protectedProcedure.query(async () => {
      ...
      const monthlyData: Record<
        string,
        { month: string; deliveries: number; volume: number }
      > = {};
      ...
      .input(
        z.object({
          ...
          data: z.object({
            name: z.string().optional(),
            type: z
              .enum(["mixer", "pump", "truck", "excavator", "crane", "other"])
              .optional(),
            status: z
              .enum(["operational", "maintenance", "repair", "inactive"])
              .optional(),
  ...
  machineWorkHours: router({
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ input }) => {
        return await db.createMachineWorkHour({
          ...input,
          hours: (input as any).hoursWorked || 0,
        } as any);
      }),
  }),
