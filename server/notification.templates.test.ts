...
describe("Notification Templates Router", () => {
  ...
  describe("getConditionOptions", () => {
    ...
    it("should include stock_quantity field", async () => {
      const options = await caller.notificationTemplates.getConditionOptions();
      const stockField = options.fields.find((f: any) => f.id === "stock_quantity");
  ...
  describe("Template CRUD", () => {
    ...
    it("should create a new template", async () => {
      const result = await caller.notificationTemplates.createTemplate({
        ...
        bodyHtml: "<p>Hello {{user_name}}, stock is low for {{material_name}}</p>",
    ...
    it("should get a specific template by ID", async () => {
      ...
      const template = await caller.notificationTemplates.getTemplate({ id: createdTemplateId });
    ...
    it("should delete a template", async () => {
      ...
      const result = await caller.notificationTemplates.deleteTemplate({ id: createdTemplateId });
  ...
  describe("Trigger CRUD", () => {
    ...
    it("should get a specific trigger by ID", async () => {
      ...
      const trigger = await caller.notificationTemplates.getTrigger({ id: createdTriggerId });
      
      expect(trigger).toBeDefined();
      expect(trigger.name).toBe("Low Stock Alert Trigger");
    });
    ...
    it("should delete a trigger", async () => {
      ...
      const result = await caller.notificationTemplates.deleteTrigger({ id: createdTriggerId });
    ...
    it("should clean up test template", async () => {
      ...
      const result = await caller.notificationTemplates.deleteTemplate({ id: templateId });
      expect(result.success).toBe(true);
    });
  });
  ...
  describe("Condition Builder Options", () => {
    it("should have operators for number fields", async () => {
      const options = await caller.notificationTemplates.getConditionOptions();
      const numberOperators = options.operators.filter((o: any) => o.types.includes("number"));
    ...
    it("should have operators for select fields", async () => {
      const options = await caller.notificationTemplates.getConditionOptions();
      const selectOperators = options.operators.filter((o: any) => o.types.includes("select"));
    ...
    it("should have select fields with options", async () => {
      const options = await caller.notificationTemplates.getConditionOptions();
      const selectFields = options.fields.filter((f: any) => f.type === "select");
